...
export default async function BottomBar() {
  ...
  return (
    <div className='fixed bottom-0 left-0 right-0 z-10 flex w-full items-center justify-center p-2 sm:bottom-full sm:top-9 sm:justify-between sm:p-0'>
      <div className='hidden flex-col bg-white/80 p-2 pl-4 font-semibold text-green-900 backdrop-blur-sm dark:bg-slate-950/80 dark:text-green-400 sm:flex sm:w-full'>
        <Link href={"/"}>
      ...
      <div className='flex w-full items-center justify-around space-x-2 rounded-3xl bg-white/80 p-2 shadow-lg backdrop-blur-sm dark:bg-slate-950/80 sm:justify-end sm:rounded-none sm:p-2 sm:shadow-none'>
        ...
        {session ? (
          <ButtonLayout href='/profile'>
            <UserCircleIcon className='h-6 w-6 ' />
            <p>프로필</p>
          </ButtonLayout>
        ) : (
          <ButtonLayout href='/auth/signin'>
