...
export default function AccessCode({ code }: { code: string }) {
  ...
  return (
    <section className='fixed bottom-0 left-0 right-0 top-0 z-10 flex h-screen w-full touch-none items-center justify-center bg-slate-100/80 p-8'>
      <div className='flex w-full max-w-xl flex-col items-center justify-center rounded-lg bg-white p-4 shadow-lg '>
        ...
        <div className='rounded-lg bg-slate-100 p-4 text-2xl font-semibold'>
          {code}
        </div>
        <div className='mt-4 flex w-full flex-col items-center justify-center'>
          ...
          <div
            className={`mt-1 rounded-lg bg-slate-100 p-1 px-2 text-sm transition ${
              copy ? "" : "hidden"
            }`}
          >
            {copy}
